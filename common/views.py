...
def send_to_telegram(name, attending, transfer, home, drinks, message):
...
class Invitation(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            logger.debug(f"Отправка анкеты: {name}, присутствие: {attending}, трансфер: {transfer}, жилье: {home}, напитки: {drinks}, сообщение: {message}")

            # Отправляем в Telegram
            if send_to_telegram(name, attending, transfer, home, drinks, message):
